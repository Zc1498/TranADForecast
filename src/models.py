...
class TranADForecast(TranAD):
	def __init__(self, feats):
		super().__init__(feats)
		print(f"初始化特征数: {feats}")
		self.name = 'TranADForecast'
		self.pred_steps = 10
		# 维度适配层（保持与原始TranAD的2*feats结构兼容）
		self.dim_expand = nn.Linear(feats, 2 * feats)
		self.dim_reduce = nn.Linear(2 * feats, feats)
		self.error_mu = nn.Parameter(torch.zeros(feats), requires_grad=False)
		self.error_sigma = nn.Parameter(torch.ones(feats), requires_grad=False)

	def predict_future(self, src, tgt):
		device = next(self.parameters()).device

		# 输入维度验证
		print(f"输入维度 src: {src.shape} tgt: {tgt.shape}")  # 调试输出
		assert src.shape[-1] == self.dim_expand.in_features, \
			f"特征数应为{self.dim_expand.in_features}，实际得到{src.shape[-1]}"

		# 维度调整（保持特征数不变）
		src = src.permute(1, 0, 2).to(device)  # [seq, batch, feats]
		tgt = tgt.permute(1, 0, 2).to(device)  # [1, batch, feats]
		tgt = self.dim_expand(tgt)  # [1, batch, 2*feats]

		predictions = []
		for _ in range(self.pred_steps):
			with torch.no_grad():
				_, x2 = super().forward(src, tgt)
				pred = self.dim_reduce(x2)  # [1, batch, feats]
				# 滚动更新时保持序列维度
				tgt = torch.cat([tgt, self.dim_expand(pred)], dim=0)[1:]
			predictions.append(pred.permute(1, 0, 2))  # [batch, 1, feats]
		return torch.cat(predictions, dim=1)  # [batch, steps, feats]

	def calculate_probability(self, src, tgt):
		device = next(self.parameters()).device

		# 输入维度强制校验
		assert src.dim() == 3 and tgt.dim() == 3, \
			f"输入应为3D张量，实际维度: src={src.dim()} tgt={tgt.dim()}"
		assert src.shape[0] == tgt.shape[0], "batch_size不匹配"
		assert src.shape[2] == self.dim_expand.in_features, \
			f"src特征数错误 预期:{self.dim_expand.in_features} 实际:{src.shape[2]}"

		# 维度转换
		src = src.permute(1, 0, 2).to(device)  # [seq, batch, feats]
		tgt = self.dim_expand(tgt.permute(1, 0, 2))  # [1, batch, 2*feats]

		# 生成预测序列
		future = self.predict_future(src, tgt)
		full_seq = torch.cat([tgt, future], dim=0)

		# 重构并计算误差
		_, recon = super().forward(src, full_seq)
		error = torch.abs(full_seq - recon)[-self.pred_steps:]

		# 标准化误差并转换为概率
		normalized = (error - self.error_mu) / (self.error_sigma + 1e-9)
		return (1 - torch.exp(-0.5 * normalized ** 2)).mean(dim=2).squeeze().cpu().numpy()